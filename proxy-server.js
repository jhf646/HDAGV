...
app.post('/api/task-log', async (req, res) => {
  try {
    const { reqCode, taskTyp, positionCodes, rawBody, respStatus, respBody } = req.body || {};
    const p = await getPool();
    await p.request()
      .input('req_code',       sql.NVarChar(100),        reqCode || '')
      .input('task_typ',       sql.NVarChar(50),         taskTyp || '')
      .input('position_codes', sql.NVarChar(sql.MAX),    typeof positionCodes === 'string' ? positionCodes : JSON.stringify(positionCodes || []))
      .input('raw_body',       sql.NVarChar(sql.MAX),    typeof rawBody === 'string' ? rawBody : JSON.stringify(rawBody || {}))
      .input('resp_status',    sql.Int,                  respStatus || 0)
      .input('resp_body',      sql.NVarChar(sql.MAX),    respBody || '')
      .query(`
